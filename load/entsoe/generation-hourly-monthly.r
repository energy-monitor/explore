...
d.agg = d.base[AreaName == "AT CTY" & ResolutionCode == "PT15M", .(
    ...
    year = year(DateTimeHourly), 
    month = month(DateTimeHourly),
...
# Agg
d.agg.group = d.agg[, .(value = sum(value)), by = .(year, month, hour, source.group)]
...
d.agg.group = d.agg.group[order(year, month, hour, source.group)]

# - STORAGE --------------------------------------------------------------------
saveToStorages(d.agg.group, list(
    id = "electricity-generation-hourly-monthly-g1",
    source = "entsoe",
    format = "csv",
