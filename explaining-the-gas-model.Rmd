...
Seit Jahresbeginn liegt der Verbrauch in Summe um `r round(savings.last.year)` % unter dem Schnitt `r START.YEAR`-`r END.YEAR`. Der Gasverbrauch liegt damit sogar niedriger als im Lockdown Jahr 2020.

```{r figure1}
relative.savings %>%
    mutate(last.year = ifelse(year == max(year), "letztes jahr", "anderes jahr")) %>% 
    ggplot(aes(x = year, y = rel.save)) +
    geom_bar(stat = "identity", aes(fill = last.year)) +
    geom_abline(slope = 0, intercept = 0) +
    xlab("") +
    ylab("") +
    labs(title = "Unterschied Gasverbrauch", subtitle = "nach Jahren, in % Änderung zum Durchschnitt 2017 - 2021")  +
  theme(legend.position="none") +
    scale_fill_manual(values = COLORS2)

```

Es gibt aber einen Zusammenhang mit der Außentemperatur: kalte Temperaturen erhöhen den Verbrauch. In einem warmen Jahr wird weniger verbraucht. Die Temperaturabhängigkeit wird dabei oft in Heizgradtagen gemessen - das ist grob gesprochen die Summe an Temperaturen für Tage, an denen geheizt werden muss. Diese war in 2022 vergleichsweise niedrig.
...
relative.hdd %>%
 ...
 #   xlab("Jahr") +
    ylab("") +
    xlab("") +
    labs(title = "Unterschied Heizgradtage", subtitle = "nach Jahren, in % Änderung zum Durchschnitt 2017 - 2021") +
    scale_fill_manual(values = COLORS2) +
  theme(legend.position="none")
...
Den Zusammenhang zwischen Temperatur und Gasverbrauch sieht man sehr schön in einem Scatterplot von Temperatur gegen Gasverbrauch. Wenn die Temperatur unter 13°C fällt, wird begonnen, zu heizen - und das wirkt auf den Gasverbrauch.

```{r figure3, fig.height=5}
d.base %>%
    filter(year >= START.YEAR) %>% 
    filter(day < max.day$max.day) %>% 
    mutate(temp.lower.13 = ifelse(temp <= 13, "<=13°C", ">13°C")) %>% 
    ggplot(aes(x = temp, y = value)) +
        geom_point(aes(col = temp.lower.13), alpha = 0.2) +
        geom_smooth(method = "lm", aes(col = temp.lower.13)) +
        xlab("Temperatur") +
        # ylab("Täglicher Gasverbrauch \n(TWh)") +
        xlab(NULL) + ylab(NULL) +
        labs(title = "Täglicher Gasverbrauch", subtitle = "nach Jahren, in TWh") +
        scale_color_manual(values = rev(COLORS2), name = "Temperatur") +
        facet_wrap(.~year) 

```

Diese Information kann man nutzen, um zu schätzen, wie hoch der Verbrauch in Österreich in einem "normalen" Jahr gewesen wäre. Man schätzt dazu den Einfluss von Temperatur, Feier-, und Wochentagen und Ferien auf den Verbrauch für eine Vergleichsperiode (wir verwenden 2017-2021). Damit simulieren wir in Folge wie hoch der Verbrauch in Österreich unter 2017-2021 Bedingungen mit dem Klima von 2022 gewesen wäre (Simulation) und vergleichen mit der Beobachtung. Um Schwankungen zu verringern, mitteln wir die beiden Zeitreihen über ein 14-Tage Zeitfenster. Während zwischen Mai und Mitte Oktober die Beobachtung immer unter der Simulation ist, ist sie ab Mitte Oktober, Anfang November fast gleich. Das würde darauf hindeuten, dass der niedrige Gasverbrauch im Oktober vor allem witterunsbedingt zu Stande gekommen ist.
...
train.years = c(START.YEAR:END.YEAR)
...
d.pred.non.economic %>%
    ...
    ggplot(aes(x = date, y = difference)) +
        ...
        labs(title = "Einsparung Gasverbrauch (mit Stromproduktion)", subtitle = "Relative Differenz zwischen Schätzung und Observation, in %")
```

Nun, ein beträchtlicher Anteil des österreichischen Gaskonsums geht in die Stromproduktion. Rechnet man diesen Anteil heraus, so ergeben sich sehr viel höhere Einsparungseffekte - die hohen Einsparungen bei Haushalten und Industrie werden also durch im Vergleich relativ hohe Verstromung von Gas in diesem Jahr reduziert. Während die Einsparungen im Oktober also etwas zurückgegangen sind, sind sie noch immer beträchtlich, wenn man den Stromsektor nicht berücksichtigt.
...
d.pred.non.economic.no.power %>%
    ...
    ggplot(aes(x = date, y = difference)) +
        ...
        scale_x_date(date_labels = "%b",date_breaks  ="1 month") +
        scale_y_continuous(n.breaks = 10) + 
        labs(title = "Einsparung Gasverbrauch (ohne Stromproduktion)", subtitle = "Relative Differenz zwischen Schätzung und Observation, in %")
```

```{r savingsFinal}
mean.gas.no.power = d.base %>% 
    filter(year >= START.YEAR & year <= END.YEAR) %>% 
    filter(day <= max.day$max.day) %>% 
    group_by(year) %>% 
...
relative.savings.no.power = d.base %>% 
    ...
    group_by(year) %>% 
    filter(day <= max.day$max.day) %>% 
    summarize(rel.save = 100 * (sum(value.without.power)-mean.gas.no.power$sum.val.mean)/mean.gas.no.power$sum.val.mean)

relative.savings.predicted = d.pred.non.economic %>% 
    filter(year >= START.YEAR) %>% 
    group_by(year) %>% 
    filter(day <= max.day$max.day) %>% 
    summarize(rel.save = 100 * (sum(prediction)-mean.gas$sum.val.mean)/mean.gas$sum.val.mean)

relative.savings.no.power.predicted = d.pred.non.economic.no.power %>% 
    filter(year >= START.YEAR) %>% 
    group_by(year) %>% 
    filter(day <= max.day$max.day) %>% 
    summarize(rel.save = 100 * (sum(prediction)-mean.gas.no.power$sum.val.mean)/mean.gas.no.power$sum.val.mean)

savings.last.year.no.power = relative.savings.no.power %>% 
    filter(year == LAST.YEAR) 

savings.last.year.predicted = relative.savings.predicted %>% 
    filter(year == LAST.YEAR) 

savings.last.year.no.power.predicted = relative.savings.no.power.predicted %>% 
    filter(year == LAST.YEAR)
```

Wie hoch sind also die Einsparungen insgesamt für Jahr 2022? Nun, beobachtet wurde eine Veränderung im gesamten Gasverbrauch von `r round(savings.last.year$rel.save)`%. Betrachtet man aber den Verbrauch ohne das Gas, welches für die Verstromung verwendet wird, so betragen die beobachteten Einsparungen `r round(savings.last.year.no.power$rel.save)`%. Simulieren wir aber mit dem Klima von 2022 ein "normales" Jahr, so würden wir nur eine Reduktion von `r round(savings.last.year.predicted$rel.save)`% erwarten, wenn die Gasverstromung inkludiert ist. Ohne Gasverstromung würden wir auf Grund des Klimas eine Reduktion von nur `r round(savings.last.year.no.power.predicted$rel.save)`% aus. In Summe gehen wir daher davon aus, dass bei Haushalten, im Gewerbe und in der Industrie beträchtliche Einsparungen gelungen sind, die Gasverstromung allerdings vergleichsweise hoch ist.
...
relative.savings.no.power %>% 
    ...
    ggplot(aes(x = Gasverbrauch, y = rel.save)) +
        ...
        scale_fill_manual(values = COLORS2) +
        xlab(NULL) + ylab(NULL) + 
        labs(title = "Veränderung Gasverbrauch 2022", subtitle = "im Vergleich zu 2017 - 2021, in %")
```
In Summe gehen wir daher davon aus, dass bei Haushalten, im Gewerbe und in der Industrie beträchtliche Einsparungen gelungen sind, die Gasverstromung allerdings vergleichsweise hoch ist. Die Grafik zeigt, dass kumulativ über das ganze Jahr die Stromproduktion aus Gas bisher im Jahr 2022 höher war als in allen anderen Jahren in der Periode 2017-2021. Das ist vor allem der niedrigen Wasserkraftproduktion im Sommer 2022 geschuldet.

```{r figureSavings}

d.power.gas = loadFromStorage(id = "electricity-generation-g1")[country == "AT" & source.group == "Gas", .(
        date = as.Date(date), value = value
    )]
...
d.power.gas %>% 
    ...
    ggplot(aes(x = day, y = cum.value)) +
    geom_line(aes(col = as.character(year))) +
    xlab(NULL) + ylab(NULL) + 
        labs(title = "Kumulative tägliche Stromproduktion aus Gas", subtitle = "nach Jahren, in TWh") +
    scale_color_manual(values = COLORS6, name = "Jahr")
