...
one.prediction = function(year.select, d.hdd, d.base, start.date) {
    ...
    d.prediction = d.prediction[, `:=`(date = date + 365)][
        (date >= (start.date)) & (date < "2023-04-01")
    ][, .(date)]
    ...
    d.prediction = merge(d.prediction, holidays, by = "date")[, .(
        date, wday, is.holiday, vacation.name
    )]
...
pred.update.daily = bind_rows(
    mapply(predict.one.day, list(2022), list(d.hdd), list(d.base), updating.period, SIMPLIFY = FALSE)
) %>%
