...
storage.at.max = storage.at %>% 
    summarize(value = max(gasInStorage, na.rm = TRUE))
...
proportion.domestic.international = (storage.start.domestic) / (storage.at.max$value -  storage.start.strategic)

change.in.storage = (storage.at.max$value - storage.at.last$value) * proportion.domestic.international
