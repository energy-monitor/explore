...
nmb.years = d.comb$year %>% unique() %>% length - 1

d.long.term.average.month = d.comb[day < max.day][(date <= "2022-01-01"), 
                             .(variable = "value.mean", 
                               v = sum(value) / nmb.years), by=c("month")][month > 2]

d.long.term.average.year = d.comb[day < max.day][month>2][(date <= "2022-01-01"), 
                             .(variable = "value.mean", 
...
setnames(d.comp, c('Monat', 'zum Vorjahr',glue('zum Zeitraum {START.YEAR}-{END.YEAR}'), 'in Variante 1', 'in Variante 2'))
